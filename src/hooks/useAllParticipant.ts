...
export const useAllParticipant = () => {
  ...
  const getUsers = useCallback((userId:number, tournamentId: number, classes: string[]) => {
    ...
    axios
      ...
      .get<{ [key: string]: Participant[] }>(`https://18.182.187.244:8080/apiTournament/getParticipant?userId=${userId}&tournamentId=${tournamentId}&className1=${className1}&className2=${className2}`)
      .then((res) => {
