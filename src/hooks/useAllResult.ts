...
export const useAllResult = () => {
  ...
  const getResults = useCallback((userId:number, tournamentId: number, classes: string[]) => {
    ...
    axios
      ...
      .get<{ [key: string]: Participant[]}>(`http://10.0.10.182:8080/apiTournament/getpreResult?userId=${userId}&tournamentId=${tournamentId}&className1=${className1}&className2=${className2}`)
      .then((res) => {
